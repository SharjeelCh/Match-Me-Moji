...
export default function StartMenu() {
 ...
 useEffect(() => {
  const loadSounds = async () => {
   try {
    const [click] = await Promise.all([Audio.Sound.createAsync(require("../assets/sounds/click.mp3"))]);
 ...
 const handlePlayPress = () => {
  clickSound.current?.replayAsync();
 ...
 return (
  <View style={styles.container}>
   ...
   <LinearGradient colors={["#1a1a2e", "#0f3460", "#16213e"]} style={StyleSheet.absoluteFill} />
   ...
   <Animated.View style={[styles.titleContainer, { transform: [{ scale: titleScale }] }]}>
    ...
    <Text style={styles.title}>Match it Moji</Text>
    <Text style={styles.subtitle}>Test Your Memory!</Text>
   </Animated.View>

   {/* Play button */}
   <TouchableWithoutFeedback onPressIn={handlePressIn} onPressOut={handlePressOut} onPress={handlePlayPress}>
